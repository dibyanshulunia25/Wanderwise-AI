...
import { Calendar, User2, Wallet } from "lucide-react";
import {
  useMotionValueEvent,
  useScroll,
  useTransform,
    ...
    >
      <div className="max-w-7xl mx-auto py-5 px-4 md:px-8 lg:px-10">
        <h2 className="text-lg md:text-4xl mb-4 text-black dark:text-white max-w-4xl">
          Your Itinerary from <strong className="text-primary">{tripData?.origin}</strong> to <strong className="text-primary">{tripData?.destination}</strong> is Ready 🎉
        </h2>
        <div className="flex items-center gap-4">
          <div className="flex gap-2">
            <Calendar className="text-primary" />
            <strong className="text-primary">{tripData?.duration}</strong>
          </div>
          <div className="flex gap-2">
            <Wallet className="text-primary" />
            <strong className="text-primary">{tripData?.budget}</strong>
          </div>
          <div className="flex gap-2">
            <User2 className="text-primary" />
            <strong className="text-primary">{tripData?.group_size}</strong>
          </div>
        </div>
      </div>
